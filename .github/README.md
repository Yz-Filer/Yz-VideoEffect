...
-	Yz-VideoEffectは動画用でコマンドラインツールとなります。  
  ...
  など、工夫次第で一発受けくらいは狙えるかもしれません。  
-	Yz-ImageEffectは静止画用でGUIツールとなります。  
  （コマンドライン引数として静止画のフルパスを渡すことができます）  
  友達の写真や飲み会の写真、旅行時の風景やスナップ写真など、印刷しないで保存しているような写真でも絵画っぽくして飾ると特別な思い出（のような感じ）になるかもしれません。
...
- レジストリは使用してません。
- 任意のディレクトリに、「Yz-VideoEffect_1.1.zip」を解凍してください。
...
:--- | :--- | :---: | :---: | :--- 
-m | --effect_mode | true | all | 画像効果の種類を数字で指定<br>Effect mode (0-6)<br> 0 : Edge<br> 1 : binarization<br> 2 : Ternarization<br> 3 : Watercolor<br> 4 : Blackboard<br> 5 : Sketch<br>6 : Anime
...
・ぼかしフィルタを1や9など強めにしてみて下さい。  
・事前に、コントラスト・輝度・彩度の調整やノイズ除去を実施してみて下さい。  
・アニメ風の場合、処理後に画像編集ソフトで彩度を上げて、青みがかるように調整してみて下さい。  
　（Photoshopの場合、色相・彩度とレンズフィルター（ブルー）のような加工）  
...
- アニメ風は静止画向けです。動画では色がコロコロ変わるのであまりお勧めできません。  
